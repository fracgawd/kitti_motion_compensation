...
kmc::Frame MakeMotionCompensationTestFrame() {
  ...
  kmc::LidarScan const scan{kmc::Time(0.0), kmc::Time(0.05), kmc::Time(0.1), cloud};
...
TEST(PsuedoTimeStampTest, XXX) {
  ...
  kmc::Time const point_1_stamp{kmc::GetPseudoTimeStamp(test_cloud.row(0), scan_start, scan_end)};
  kmc::Time const point_2_stamp{kmc::GetPseudoTimeStamp(test_cloud.row(1), scan_start, scan_end)};
  kmc::Time const point_3_stamp{kmc::GetPseudoTimeStamp(test_cloud.row(2), scan_start, scan_end)};
...
TEST(MotionCompensationTest, XXX) {
  ...
  kmc::Pointcloud motion_compensated_pointcloud{kmc::MotionCompensate(test_frame, requested_time)};
